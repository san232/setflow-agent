...
곡의 분위기 고저차를 고려해 플레이리스트를 만드는 Python 에이전트 서버입니다. 원하는 흐름을 선택하면 곡 순서와 배치 이유를 확인할 수 있습니다. 초기 보관함에는 King Gnu 43곡과 조사 초깃값이 기본으로 제공되며, YouTube에서 원하는 아티스트의 곡을 검색해 추가하거나 기존 곡에 영상 링크를 연결할 수 있습니다.
...
1. 앱을 열면 **King Gnu 43곡**이 보관함에 기본으로 들어 있습니다. 서버가 시작될 때 자동 등록하며, 기존 곡의 편집값은 덮어쓰지 않습니다. 검색으로 다른 아티스트의 곡도 추가할 수 있습니다.
...
기본 곡 자동 등록은 로컬과 배포 환경에서 모두 기본으로 켜져 있습니다. 빈 보관함으로 시작하려면 새 DB에 `SETFLOW_AUTO_SEED=false`를 설정합니다. 기본 곡을 수동으로 다시 추가할 때는 서버를 종료한 뒤 다음 CLI를 실행하고 다시 켭니다.
...
보관함 위의 **YouTube에서 곡 찾기**에 `King Gnu 白日`처럼 입력하고 검색합니다. 썸네일·제목·길이·아티스트 또는 채널을 확인한 뒤 **이 곡 선택**을 누릅니다. 기존 곡과 일치하면 편집한 분위기 수치를 유지하고 링크만 채웁니다. 새 곡은 제목·아티스트와 다섯 분위기 수치를 확인한 다음 **곡 등록**으로 저장합니다. 곡 이름이 다른 언어나 라이브 버전으로 표시될 때는 보관함에서 해당 곡의 **링크 찾기**로 연결 대상을 먼저 지정합니다.

**곡 우선**은 YouTube Music 공개 검색을 사용하고, 결과가 없거나 연결에 실패하면 일반 YouTube 영상 검색으로 전환합니다. **YouTube 영상**은 영상 검색을 바로 사용합니다. 로그인·API 키가 필요 없으며, 검색 자체는 보관함을 바꾸지 않습니다. 자연어로 `유튜브에서 King Gnu 白日 검색해줘`라고 요청하면 `search_music` 도구와 실행 로그를 확인할 수 있습니다.

영상 채널명은 실제 아티스트 이름과 다를 수 있어 새 곡의 아티스트로 자동 입력하지 않습니다. 검색 연결이 불가능하면 외부 YouTube/YouTube Music 검색 링크도 이용할 수 있습니다. 저장 후 보관함과 새로 생성한 플레이리스트의 **YouTube 열기**로 영상을 볼 수 있습니다. 계정 재생목록 자동 저장과 음원 다운로드는 제공하지 않습니다. 구현과 검증 방법은 [음악 검색 안내](docs/MUSIC_SEARCH.md)에 있습니다.
...
| 도구 실행 | 등록된 9개 도구의 인자 검증과 성공·실패 기록 |
...
등록된 도구는 `add_song`, `list_songs`, `update_song`, `delete_song`, `generate_playlist`, `get_playlist`, `explain_playlist`, `export_playlist`, `search_music`입니다. 순서 자체는 서버의 정렬 알고리즘이 계산합니다.
...
2026-09-15 YouTube 검색 추가 후 프로젝트 가상환경에서 **80 passed, 1 deselected, 1 warning in 86.65s**를 기록했습니다. Frontend의 기존 수치 보존·연결 대상·URL 검증도 통과했습니다. [검색 추가 후 테스트 XML](docs/evidence/pytest-music-search-2026-09-15.xml)에 실제 결과를 남겼습니다. 기본 테스트 설정이 `live_openai` 1개를 제외하며, 기본 실행에는 로컬 정렬·API·DB·도구 테스트와 외부 응답을 대체한 루프 테스트가 포함됩니다.
